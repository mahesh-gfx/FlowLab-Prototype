...
router.post("/execute-workflow", async (req: Request, res: Response) => {
  const workflowData: WorkflowStructure = req.body.workflowData;
  ...
  let workflowExists: boolean;
  if (workflowId != "new")
    workflowExists = await workflowService.doesWorkflowExist(
      workflowId,
      userId
    );
  else workflowExists = false;
  ...
  console.log("Workflow exists: ", workflowExists);
  if (!workflowExists) {
